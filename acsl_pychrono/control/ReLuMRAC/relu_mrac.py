...
from acsl_pychrono.control.ReLuMRAC.relu_mrac_gains import ReLuMRACGains
...
from acsl_pychrono.control.ReLuMRAC.m_relu_mrac import M_ReLuMRAC, ReLUFeatureMap
from acsl_pychrono.control.projection_operator import ProjectionOperator

class ReLuMRAC(BaseMRAC, Control):
  def __init__(self, gains: ReLuMRACGains, ode_input: OdeInput, flight_params: FlightParams, timestep: float):
    ...
    # Initiate the ReLu neural network for the regressor
  ...
  def computeControlAlgorithm(self, ode_input: OdeInput):
    ...
    self.Theta_hat_tran_ReLu = self.y[106:106 + (self.gains.NN_width * self.gains.B_ref_tran.shape[1])] # \hat{\Theta} (translational - Neural Network) 

    # Reshapes all adaptive gains to their correct (row, col) shape as matrices
    self.reshapeAdaptiveGainsToMatricesReLuMRAC(self.gains.NN_width)
    ...
    self.mu_adaptive_tran = M_ReLuMRAC.computeControlLawReLu(
    ...
    self.Moment_adaptive = M_ReLuMRAC.computeControlLaw(
  ...
  def ode(self, t, y):
    ...
    self.dy[106:106 + (self.gains.NN_width * self.gains.B_ref_tran.shape[1])] = self.Theta_hat_tran_ReLu_dot.reshape((self.gains.NN_width * self.gains.B_ref_tran.shape[1]), 1)
  ...
  def updateAdaptiveLawsOuterLoop(self):
    ...
    ) = M_ReLuMRAC.compute_eTransposePB(self.e_tran, self.gains.P_tran, self.gains.B_tran)

    # Deadzone modification Outer Loop
    self.dead_zone_value_tran = M_ReLuMRAC.deadZoneModulationFunction(
    ...
    (self.K_hat_x_tran_dot,
     ...
     self.Theta_hat_tran_ReLu_dot
    ) = M_ReLuMRAC.computeAllRobustAdaptiveLawsReLu(
      ...
      self.gains.sigma_x_tran, self.gains.sigma_r_tran, self.gains.sigma_Theta_tran, self.gains.sigma_Theta_tran_ReLu,
      eTranspose_P_B_norm_tran,
      self.K_hat_x_tran, self.K_hat_r_tran, self.Theta_hat_tran, self.Theta_hat_tran_ReLu,
  ...
  def updateAdaptiveLawsInnerLoop(self):
    ...
    ) = M_ReLuMRAC.compute_eTransposePB(self.e_rot, self.gains.P_rot, self.gains.B_rot)

    # Deadzone modification Inner Loop
    self.dead_zone_value_rot = M_ReLuMRAC.deadZoneModulationFunction(
    ...
    ) = M_ReLuMRAC.computeAllRobustAdaptiveLaws(
